...
class tweet_labelling:
    ...
    def textSimilarity(self):
        ...
        for i, weight in enumerate(set_of_TF):
            ...
            term_frequency.append(weight)
        
        #output a frame containing each tweet, the assigned TF-IDF weights, the class and original tweet for cross checking                
        d = pd.DataFrame({'Username': Usernames, 'Weights':tweets, 'Original_tweets': original_tweets,'Term-frequency-weight':term_frequency, 'Class':set_of_TF}) 
        d = d[['Username', 'Weights', 'Original_tweets','Term-frequency-weight', 'Class']]
        d.set_index('Username', inplace=True)
        d.to_csv('diabetes\\diabetes-training-set.csv')     
    
    #function searches and returns the total number of unique users
    def unique_users(self):
        no_unique_users = []
        for i in self.__file['Username']:
            if (i not in no_unique_users):
                no_unique_users.append(i)
        return (len(no_unique_users))

            
fh = pd.read_csv('diabetes\\diabetes-tweets-analysis.csv')
syns_advise = wordnet.synsets('advise')[1].name()
sentiment = tweet_labelling(fh, syns_advise)
sentiment.textSimilarity()